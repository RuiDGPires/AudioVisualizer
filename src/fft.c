 ...
 */
void
_fft( complex *v, usize n, complex *tmp )
{
 ...
 */
void
ifft( complex *v, usize n, complex *tmp )
{
  if(n>1) {			/* otherwise, do nothing and return */
    int k,m;    complex z, w, *vo, *ve;
    ve = tmp; vo = tmp+n/2;
...
void
fft(i32 *buffer, i32 *out, usize size) {
  complex v[size], tmp[size];
  for (usize i = 0; i < size; i++) {
    v[i] = (complex){.Re = buffer[i], .Im = 0};
  }
